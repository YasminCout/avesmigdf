...
setwd("C:/Tflavipes/Localities/")
#Get data points
flavipes <- read.csv(file = "tflavipes_corrigido.csv", header = T);
flavipes <- cbind(flavipes, rep.int(1, length(nrow(flavipes)))); #Adds another column indicating these are presence points
colnames(flavipes) <- c("X", "Y", "Response");

#Get environmental variables
setwd("C:/Tflavipes/Ms/Crowned/");
...
setwd("C:/Tflavipes/EnvironmentalData/Future/ssp585/2080_2100/")
...
bmData <- BIOMOD_FormatingData(resp.var = flavipes[,3],
                               resp.xy = flavipes[,1:2], 
                               resp.name = "Tflavipes",
                               expl.var = envt.st,
                               PA.nb.rep=1
);
...
#Running Maxent
setwd("C:/Tflavipes/TestModelRun/R/")
...
coordinates(melanoleuca) <- ~X + Y
plot(presentResult, main = "Presente - Tringa Flavipes");
plot(melanoleuca, pch = 17,col = 24, add = TRUE)
writeRaster(presentResult, filename = "Tflavipesresent", format = "GTiff", overwrite = T);
plot(melanoleuca)
...
plot(presentEnsembleResult, main = "Presente - Tringa Flavipes");
writeRaster(presentEnsembleResult, filename = "TflavipesPresent", format = "GTiff", overwrite = T);
...
plot(result2100, main = "Em 2100 - Tringa Flavipes");

writeRaster(result2100, filename = "Tflavipes2100", format = "GTiff", overwrite = T);
...
plot(ensembleResult2100, main = "Em 2100 - Tringa Flavipes");
writeRaster(ensembleResult2100, filename = "Tflavipes2100Ensemble", format = "GTiff", overwrite = T);
...
forSetup <- read.csv(file = paste("C:/Tflavipes/TestModelRun/R/Tflavipes/models/1605399367/Tflavipes_PA1_RUN1_MAXENT.Phillips_outputs/maxentResults.csv", sep = ""), header = T)#Choose the appropriate model folder with the seed of the analysis you want
...
count <- 1;
while (count <= 10){
  temporary <- read.csv(file = paste("C:/Tflavipes/TestModelRun/R/Tflavipes/models/1605399367/Tflavipes_PA1_RUN", count, "_MAXENT.Phillips_outputs/maxentResults.csv", sep = ""), header = T);
