...
UTM_proj = CRS("+proj=utm +zone=23 +south +ellps=WGS84 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs")
...
pontos_brutos = read.csv("Localities/GryseoAur.csv")
head(pontos_brutos)
str(pontos_brutos)

#Projetar os pontos de long e lat:
plot(pontos_brutos$longitude, pontos_brutos$latitude)

#salvar apenas long e lat como novo objeto SpatialPoints
names(pontos_brutos)
longlat = pontos_brutos[, c(2:3)]
coordinates(longlat) = ~longitude+latitude #similar to SpatialPoints
...
america = shapefile("Localities/South_America.shp")
#verificar se carregou corretamente
plot(America)
class(America)

#verificar a tabela de atributos do shapefile:
names(america@data) #nomes das colunas
length(america@data[,1]) #numero de linhas/poligonos independentes
head(america@data) # verificar o conteudo da tabela
...
writeOGR(america, "~/Crowned/mascaras", "mascara_SA", driver="ESRI Shapefile", overwrite_layer = T)
